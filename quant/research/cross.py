...
def get_stock_mom(rm, lookback=8):
    m = rm.rolling(lookback, min_periods=4).sum()
...
def calc_blind_momentum(rtn, rm, vol):
    ...
    ans = pd.concat(ans, axis=1).T
    ans2 = pd.concat(ans2, axis=1).T
    ans3 = pd.concat(ans3, axis=1).T
    ans4 = pd.concat(ans4, axis=1).T
    ans = ans[~rm.isnull()]
    ans2 = ans2[~rm.isnull()]
    ans3 = ans3[~rm.isnull()]
    ans4 = ans4[~rm.isnull()]
    stm = -1. * np.sign(ans.subtract(ans.mean(axis=1), axis=0)).divide(vol)
    ltm = np.sign(ans2.subtract(ans2.mean(axis=1), axis=0)).divide(vol)
    stm2 = -1. * np.sign(ans3.subtract(ans3.mean(axis=1), axis=0)).divide(vol)
    ltm2 = np.sign(ans4.subtract(ans4.mean(axis=1), axis=0)).divide(vol)
    s = rtn.mul(stm.shift()).sum(axis=1) / stm.abs().sum(axis=1).shift()
    l = rtn.mul(ltm.shift()).sum(axis=1) / ltm.abs().sum(axis=1).shift()
    s2 = rtn.mul(stm2.shift()).sum(axis=1) / stm2.abs().sum(axis=1).shift()
    l2 = rtn.mul(ltm2.shift()).sum(axis=1) / ltm2.abs().sum(axis=1).shift()
    return stm, ltm, s, l, s2, l2
...
def get_long_pos(rtn, rm, vol, x, x2, i, start_date, high, low, style='A', enhance=True):
    if style == 'A':
        l = np.arange(0., 1.51, .1)
        s1 = rm.rolling(i, min_periods=1).mean().loc[rtn.index]
        s2 = rm.rolling(52, min_periods=13).mean().shift(i).loc[rtn.index]
    elif style == 'B':
        l = np.arange(1., 2.51, .1)
        s1 = rm.rolling(i, min_periods=1).mean().loc[rtn.index] * np.sqrt(1. * i)
        s2 = rm.rolling(52, min_periods=13).mean().shift(i).loc[rtn.index] * np.sqrt(52.)
    acc = rtn.cumsum()
    ax = acc.rolling(i, min_periods=1).min()
    s1 = s1[acc == ax]
    s2 = s2[acc == ax]
    return get_pos(s1, s2, vol, high, low, x, x2, enhance)[start_date:]
...
def test_reversal(universe='SMX', start_date=dt(2009, 1, 1), end_date=dt(2015, 12, 31), style='A', enhance=True):
    ...
    if params is not None:
        ...
        for i in xrange(1, 13):
            ...
            pos = get_long_pos(rtn, rm, vol, x, x2, i, start_date, high, low, style, enhance)
            pr = pos.mul(tu.fit_data(fr, pos)).fillna(0.)
            xx = tu.get_observations(rtn.rolling(52, min_periods=13).mean().shift(i).fillna(0.), pos)
            y = tu.get_observations(pr, pos)
            vu.binned_statistic_plot(xx, y, 'mean', 7, (-.05, .05))
